...
function four_corners_scripts() {
	...
	wp_enqueue_style( 'four_corners_style', 'https://cdn.jsdelivr.net/npm/@four-corners/fourcorners.js@0.9.3/dist/fourcorners.min.css' );
...
function get_local_page( $slug ) {
	...
	$page_id = $page->ID;
	$local_id = function_exists('pll_get_post') && function_exists('pll_get_post') ?
		pll_get_post( $page_id, pll_get_post() )
	: null;
...
function get_lang_list( $args = array() ) {
...
function get_lang_post( $id, $lang ) {
...
function page_endpoint( $req ) {
	...
	$slug = $req['slug'];
	$lang = isset( $req['lang'] ) && function_exists( 'pll_default_language' ) 
		? $req['lang']
